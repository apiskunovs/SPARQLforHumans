...
namespace SparqlForHumans.Server.Controllers
{
    ...
    public class FindIndividualsController : Controller
    {
        private readonly NLog.Logger _logger = Logger.Logger.Init();
        private readonly LuceneConfig _luceneConfig;

        public FindIndividualsController(LuceneConfig luceneConfig)
        {
        ...
        {
            _logger.Info("FindIndividualsController Started");
            _logger.Info($"Params words={words}, instanceOf={instanceOf}, inProp={inProp}, outProp={outProp}, limit={limit},, id={id}, idNot={idNot}");
            var filteredItems = new FindInstancesQuery(this._luceneConfig.EntityIndexPath, words, instanceOf, inProp, outProp, id, idNot, false.ToString(), limit).Query();
            _logger.Info("FindIndividualsController Ended");
